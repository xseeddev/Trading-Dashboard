...
class Utils:
    ...
    def isTodayWeeklyExpiryDay():
        ...
        print("today's date: " + str(todayDate) +
              "  weekly expiry: " + str(expiryDateTime))
    ...
    def trade_req(angel, client, buy_leg_strike, sell_leg_strike, option_type, expiry):
        try:
            with file_lock:
                script_list = pd.read_csv(
                    r'script_master.csv', low_memory=False)
            buy_leg_token, buy_leg_sym = Utils.getTokenInfo(
                df=script_list, strike_price=buy_leg_strike, option_type=option_type, expiry_pref=expiry)
            sell_leg_token, sell_leg_sym = Utils.getTokenInfo(
            ...
            marigin_per_lot = Utils.req_margin(
                angel, buy_leg_token, sell_leg_token)
    ...
    def update_nf_ltp(angel, i=[0]):
        try:
            nf_cmp = angel.ltpData("NSE", "Nifty 50", "99926000")[
                'data']['ltp']
            i[0] = 0  # Reset i to 0
            return nf_cmp
        except:
    ...
    def write_log(message):
        try:
            with file_lock:
                prev_logs = json.load(
                    open(r'C:\algo\backend\activity_log.json', 'r'))
                if datetime.now().strftime("%Y-%d-%m") not in prev_logs:
                    prev_logs[datetime.now().strftime(
                        "%Y-%d-%m")] = [datetime.now().strftime("%H:%M:%S") + "  " + message]
                else:
                    prev_logs[datetime.now().strftime(
                        "%Y-%d-%m")].append(datetime.now().strftime("%H:%M:%S") + "  " + message)
                with open(r'C:\algo\backend\activity_log.json', 'w') as f:
                    json.dump(prev_logs, f, indent=4)
        except:
            print("write_log failed" + datetime.now().strftime("%H:%M:%S"))
            traceback.print_exc()
